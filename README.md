...
- 本项目仅供学习和个人使用，为避免版权纠纷，必须设置PASSWORDS环境变量
- 请勿将部署的实例用于商业用途或公开服务
...
4. 使用以下设置：
   - 构建命令：留空（无需构建）
   - 输出目录：留空（默认为根目录）
5. **⚠️ 重要：在"设置" > "环境变量"中添加 `PASSWORDS` 变量（必须设置）**
6. 点击"保存并部署"
...
3. 导入您的仓库，使用默认设置
4. **⚠️ 重要：在"Settings" > "Environment Variables"中添加 `PASSWORDS` 变量（必须设置）**
5. 点击"Deploy"
...
services:
  libretv:
    ...
    environment:
      - PASSWORDS=${PASSWORDS:-111111} # 可将 111111 修改为你想要的密码，默认为 your_password
    restart: unless-stopped
```
启动 LibreTV：
...
**重要提示**: 为确保安全，所有部署都必须设置 PASSWORDS 环境变量，否则用户将看到设置密码的提示。
